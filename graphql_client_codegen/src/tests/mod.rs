...
#[test]
fn schema_with_fragments_generate_unknown_works() {
    let query_string = include_str!("fragments_query.graphql");
    let query = graphql_parser::parse_query(query_string).expect("Parse fragments query");
    let schema = graphql_parser::parse_schema(include_str!("fragments_schema.graphql"))
        .expect("Parse fragments schema");
    let schema = Schema::from(schema);
    ...
    for (_id, operation) in query.operations() {
        ...
        .to_token_stream()
        .expect("Generate fragments module");
        let generated_code = generated_tokens.to_string();
